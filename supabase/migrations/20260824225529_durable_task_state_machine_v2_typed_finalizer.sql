...
begin
 ...
 if r.lease_token is distinct from p_lease_token or r.lease_revoked_at is not null or r.status not in ('claimed','running') or b.status<>'running' or exists(select 1 from public.contentflow_builder_runs n where n.backlog_task_id=r.backlog_task_id and n.id>r.id) then
   ...
   return jsonb_build_object('ok',false,'reason','commit_fenced','run_id',r.id);
 end if;
...
revoke all on function public.contentflow_finalize_run_v2(bigint,text,integer,jsonb) from public,anon,authenticated;
grant execute on function public.contentflow_finalize_run_v2(bigint,text,integer,jsonb) to service_role;
