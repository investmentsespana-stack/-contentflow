alter table public.contentflow_runtime_evidence_ledger add column if not exists requirement_id bigint references public.contentflow_evidence_requirements(id);
create index if not exists contentflow_runtime_evidence_requirement_idx on public.contentflow_runtime_evidence_ledger(project_key,requirement_id,created_at desc);
...
begin
  ...
  if p_test_profile not in ('evidence-persistence','runtime-test','static-analysis') then raise exception 'unsupported_test_profile'; end if;
  ...
  values(er.project_key,er.backlog_task_id,er.source_run_id,er.task_key,
    case when p_test_profile='static-analysis' then 'static_analysis' else 'runtime_test' end,
    v_key,
...
begin
  ...
  update public.contentflow_evidence_requirements er set status='verified',verified_at=now(),updated_at=now(),
    evidence_ref=jsonb_build_object('architecture','CI_RUNTIME_EVIDENCE_BRIDGE_V1','evidence_id',e.evidence_id,'evidence_type',e.evidence_type,'evidence_key',e.evidence_key,'sha256',e.payload_sha256,'producer',e.producer,'observed_at',e.observed_at,'payload',e.payload)
  ...
  return jsonb_build_object('architecture','CI_RUNTIME_EVIDENCE_BRIDGE_V1','requirements_verified',v_verified,'evidence_tasks_completed',v_completed,'originals_reopened',v_reopened);
end $$;
