...
-- original-statements-sha256: 3139e7037717dc9eea937d6c7a0a13aaa93a3ba53c678ff53f77a218972b2b7c
-- original-version: 20260826225228
-- original-name: classify_artifact_truncation_retryable

insert into public.contentflow_retry_policies(error_class,retryable,max_attempts,initial_backoff_seconds,backoff_coefficient,max_backoff_seconds,jitter_ratio,switch_model_on_retry,notes,updated_at)
values('artifact_truncation',true,3,15,2.0,90,0.10,true,'Truncated model output is retryable with bounded backoff and model rotation; do not open a permanent unknown-error circuit.',now())
on conflict(error_class) do update set retryable=excluded.retryable,max_attempts=excluded.max_attempts,initial_backoff_seconds=excluded.initial_backoff_seconds,backoff_coefficient=excluded.backoff_coefficient,max_backoff_seconds=excluded.max_backoff_seconds,jitter_ratio=excluded.jitter_ratio,switch_model_on_retry=excluded.switch_model_on_retry,notes=excluded.notes,updated_at=now();
