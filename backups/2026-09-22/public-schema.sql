...
\restrict ssXRnYLJpo2IZm5GDEaTGlApEDvR8mDqLW8D4pxX80znzIhhTptvIYWH1WTpwQg
...
declare
  ...
  missing_tool_recipe boolean := false;
  circuit_open boolean := false;
  semantic_external_hold boolean := false;
begin
  if coalesce(new.completion_phase,'')='strategy_change_required' then
    new.status := 'deferred';
    new.blocked_reason := 'STRATEGY_CHANGE_REQUIRED';
  ...
  semantic_external_hold := coalesce(new.workflow_state,'')='external_prerequisite'
    or coalesce(new.completion_phase,'') in ('external_prerequisite','waiting_for_evidence');
  ...
  missing_tool_recipe := coalesce(new.execution_lane,'')='tool_executor' and not tool_recipe;
  ...
  if new.status='ready' then
    ...
    elsif semantic_external_hold then
      new.status := 'blocked';
      new.blocked_reason := coalesce(nullif(new.blocked_reason,''), case when coalesce(new.completion_phase,'')='waiting_for_evidence' then 'VERIFIED_SOURCE_EVIDENCE_REQUIRED' else 'EXTERNAL_PREREQUISITE_REQUIRED' end);
      new.next_eligible_at := null;
    ...
    elsif missing_tool_recipe then
      new.status := 'blocked';
      new.blocked_reason := 'EXECUTION_RECIPE_REQUIRED';
      new.workflow_state := 'contract_repair_required';
      new.next_eligible_at := null;
      new.patch_feedback := concat_ws(E'\n',nullif(new.patch_feedback,''),'STATE_GUARD_V3: tool_executor has no executable recipe; repair the contract instead of retrying.');
    else
    ...
    if coalesce(new.blocked_reason,'')='REVIEW_PENDING'
      ...
      if coalesce(new.blocked_reason,'') in ('','STATE_GUARD_BLOCKED_UNSPECIFIED','EXECUTION_RECIPE_REQUIRED') then
...
begin
 ...
 -- Only optional/legacy evidence may be obsoleted. Ordinary tool executors are
 -- real workflow dependencies and MUST NOT be removed merely because they are incomplete.
 ...
 loop
   update public.contentflow_build_backlog as t
    ...
    where t.id=rec.id
      ...
      and (
        e.epic='evidence_first'
        or e.execution_lane='evidence_producer'
        or exists(
 ...
 set status='deferred',blocked_reason='OBSOLETE_BY_DURABLE_CONTRACT_V4',updated_at=now()
 ...
 update public.director_repair_incidents i
 set status='resolved',requires_human=false,resolved_at=now(),updated_at=now(),outcome='superseded_by_durable_contract_v4',diagnosis='Legacy escalation contradicted explicit durable workflow contract',validation='contract_driven_reconciliation'
 ...
 return jsonb_build_object('architecture','DURABLE_TASK_STATE_MACHINE_V4_PRESERVE_TOOL_EXECUTOR_DEPS','reopened_patch_tasks',reopened,'obsolete_false_evidence',obsolete_evidence,'legacy_evidence_edges_removed',deps_removed,'legacy_incidents_resolved',incidents_resolved);
end
$$;
...
--
-- Name: contentflow_reconcile_preproduction_evidence_stall_v1(text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.contentflow_reconcile_preproduction_evidence_stall_v1(p_project_key text DEFAULT 'agent-academy-platform-v1'::text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'pg_temp'
    AS $$
declare v_count int:=0;
...
--
-- Name: director_browser_action_gate(text, text, text, text, jsonb); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.director_browser_action_gate(p_project_key text, p_actor text, p_domain text, p_action text, p_request_payload jsonb DEFAULT '{}'::jsonb) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
declare v_high boolean; v_id uuid; v_status text;
begin
  v_high := lower(coalesce(p_action,'')) ~ '(publish|post|delete|remove|archive|credential|password|oauth|payment|purchase|legal|accept|submit|send|change.*username|change.*email)'
    or lower(coalesce(p_domain,'')) ~ '(bank|broker|payment)';
  v_status := case when v_high then 'pending' else 'approved' end;
  insert into public.director_browser_action_audit(project_key,actor,domain,action,risk_class,destructive,requires_human_approval,approval_status,request_payload,decision_reason,decided_at)
  values (p_project_key,p_actor,p_domain,p_action,case when v_high then 'high' else 'low' end,v_high,v_high,v_status,coalesce(p_request_payload,'{}'::jsonb),
    case when v_high then 'human_approval_required' else 'non_destructive_allowlisted_action' end,case when v_high then null else now() end)
  returning id into v_id;
  return jsonb_build_object('audit_id',v_id,'allowed',not v_high,'requires_human_approval',v_high,'approval_status',v_status,'risk_class',case when v_high then 'high' else 'low' end);
end; $$;
...
--
-- Name: director_context_facts; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.director_context_facts (
    id bigint NOT NULL,
    ...
    CONSTRAINT director_context_facts_status_check CHECK ((status = ANY (ARRAY['active'::text, 'superseded'::text, 'retracted'::text])))
);


--
-- Name: TABLE director_context_facts; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: director_publish_context_fact(text, text, jsonb, text, text, timestamp with time zone, timestamp with time zone, boolean); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.director_publish_context_fact(p_project_key text, p_fact_key text, p_payload jsonb, p_source text, p_source_ref text DEFAULT NULL::text, p_observed_at timestamp with time zone DEFAULT now(), p_expires_at timestamp with time zone DEFAULT NULL::timestamp with time zone, p_verified boolean DEFAULT false) RETURNS public.director_context_facts
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
declare v_prev public.director_context_facts; v_next public.director_context_facts; v_version bigint;
begin
  if coalesce(btrim(p_project_key),'')='' or coalesce(btrim(p_fact_key),'')='' or coalesce(btrim(p_source),'')='' then raise exception 'project_key, fact_key and source are required'; end if;
  perform pg_advisory_xact_lock(hashtextextended(p_project_key || ':' || p_fact_key,0));
  select * into v_prev from public.director_context_facts where project_key=p_project_key and fact_key=p_fact_key and status='active' order by version desc limit 1 for update;
  select coalesce(max(version),0)+1 into v_version from public.director_context_facts where project_key=p_project_key and fact_key=p_fact_key;
  if v_prev.id is not null then update public.director_context_facts set status='superseded' where id=v_prev.id; end if;
  insert into public.director_context_facts(project_key,fact_key,version,status,payload,source,source_ref,observed_at,expires_at,verified,supersedes_id)
  values (p_project_key,p_fact_key,v_version,'active',coalesce(p_payload,'{}'::jsonb),p_source,p_source_ref,coalesce(p_observed_at,now()),p_expires_at,coalesce(p_verified,false),v_prev.id)
...
begin
 ...
 if coalesce(v_task.epic,'') in ('evidence_capability','evidence_capability_root','capability_bootstrap') then
   ...
   select string_agg(format('DEPENDENCY: %s | title=%s | status=%s | quality=%s\nVERIFIED RESULT:\n%s',d.task_key,coalesce(d.title,''),d.status,coalesce(d.quality_score::text,'0'),case when length(coalesce(d.result,'')) <= 9000 then coalesce(d.result,'') else left(coalesce(d.result,''),3500)||E'\\n...[DEPENDENCY_CONTEXT_TRUNCATED: DO NOT RECONSTRUCT SOURCE; USE ONLY VISIBLE CONTRACT AND VERIFIED RUNTIME EVIDENCE]...\\n'||right(coalesce(d.result,''),3500) end),E'\n---\n') into v_dep_context
   from jsonb_array_elements_text(coalesce(v_task.depends_on,'[]'::jsonb)) dep(value)
   join public.contentflow_build_backlog d on d.project_key=v_task.project_key and d.task_key=dep.value;
 end if;
...
--
-- Name: trading_sqx_director_dispatch(text, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.trading_sqx_director_dispatch(p_command_type text, p_project text DEFAULT NULL::text) RETURNS TABLE(request_id uuid, bridge_command_id uuid, state text, reason text)
    LANGUAGE sql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
...
--
-- Name: trading_sqx_director_dispatch_payload(text, jsonb); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.trading_sqx_director_dispatch_payload(p_command_type text, p_payload jsonb DEFAULT '{}'::jsonb) RETURNS TABLE(request_id uuid, bridge_command_id uuid, state text, reason text)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
declare
  ...
  v_project text := nullif(trim(coalesce(v_payload->>'project', v_payload->>'name','')), '');
  ...
  v_read_only constant text[] := array[
    ...
    'save_project_config','vibe_diagnose','stack_health'
  ];
  v_control_cmds constant text[] := array[
    'run_project','stop_project','pause_project','resume_project','load_project_config','update_data',
    'add_instrument','add_symbol','create_databank','copy_databank','move_databank','freeze_databank',
    'run_vibe_asset_research','sync_vibe_runtime','start_vibe','stop_vibe','restart_vibe'
  ];
  ...
  v_vibe_action text := lower(trim(coalesce(v_payload->>'action','')));
  v_vibe_asset text := upper(trim(coalesce(v_payload->>'asset','')));
  v_vibe_run_id text := trim(coalesce(v_payload->>'run_id',''));
begin
  ...
  elsif p_command_type='freeze_databank'
        and nullif(trim(coalesce(v_payload->>'databank','')), '') is null then
    v_reason := 'databank_required';
  elsif p_command_type='sync_vibe_runtime'
        and (jsonb_typeof(v_payload->'files') <> 'array' or jsonb_array_length(v_payload->'files')=0) then
    v_reason := 'vibe_sync_files_required';
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'bridge_version',v_device.last_health->>'bridge_version',
      'sqx_cli_ok',v_device.last_health->'sqx_cli_ok',
      'payload_keys',(select coalesce(jsonb_agg(key),'[]'::jsonb) from jsonb_object_keys(v_payload) key)
    ),
    v_reason
...
--
-- Name: trading_sqx_director_dispatch_v2(text, text, jsonb); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.trading_sqx_director_dispatch_v2(p_command_type text, p_project text DEFAULT NULL::text, p_payload jsonb DEFAULT '{}'::jsonb) RETURNS TABLE(request_id uuid, bridge_command_id uuid, state text, reason text)
    LANGUAGE sql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
  select *
  from public.trading_sqx_director_dispatch_payload(
    p_command_type,
    coalesce(p_payload,'{}'::jsonb) ||
      case
        when nullif(trim(coalesce(p_project,'')),'') is null then '{}'::jsonb
        else jsonb_build_object('project',trim(p_project))
      end
  );
$$;
...
--
-- Name: trading_sqx_rara_verify_command(); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.trading_sqx_rara_verify_command() RETURNS trigger
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
declare
...
begin
  ...
  v_semantic_error :=
    ...
    or v_text ~* 'another instance of StrategyQuant X is running'
    or v_text ~* 'SQ can run only with one instance at once'
...
--
-- Name: director_browser_action_audit; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.director_browser_action_audit (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    project_key text NOT NULL,
    actor text NOT NULL,
    ...
    CONSTRAINT director_browser_action_audit_approval_status_check CHECK ((approval_status = ANY (ARRAY['approved'::text, 'pending'::text, 'blocked'::text]))),
    CONSTRAINT director_browser_action_audit_risk_class_check CHECK ((risk_class = ANY (ARRAY['low'::text, 'high'::text])))
);


--
-- Name: TABLE director_browser_action_audit; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: director_context_facts_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--

ALTER TABLE public.director_context_facts ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME public.director_context_facts_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
--
-- Name: trading_sqx_bridge_commands; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.trading_sqx_bridge_commands (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    ...
    error text,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT trading_sqx_bridge_commands_command_type_check CHECK ((command_type = ANY (ARRAY['cli_help'::text, 'list_projects'::text, 'list_databanks'::text, 'status_project'::text, 'list_symbols'::text, 'list_instruments'::text, 'list_timezones'::text, 'count_databank'::text, 'export_databank'::text, 'list_strategies'::text, 'get_strategy_stats'::text, 'save_project_config'::text, 'run_vibe_nq6_smoke'::text, 'run_vibe_asset_research'::text, 'vibe_diagnose'::text, 'stack_health'::text, 'run_project'::text, 'stop_project'::text, 'pause_project'::text, 'resume_project'::text, 'live_project_control'::text, 'load_project_config'::text, 'update_data'::text, 'add_instrument'::text, 'add_symbol'::text, 'create_databank'::text, 'copy_databank'::text, 'move_databank'::text, 'freeze_databank'::text, 'sync_vibe_runtime'::text, 'start_vibe'::text, 'stop_vibe'::text, 'restart_vibe'::text]))),
...
--
-- Name: TABLE trading_sqx_bridge_commands; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: trading_sqx_bridge_devices; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.trading_sqx_bridge_devices (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    ...
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    revoked_at timestamp with time zone,
    CONSTRAINT trading_sqx_bridge_devices_status_check CHECK ((status = ANY (ARRAY['ACTIVE'::text, 'REVOKED'::text, 'OFFLINE'::text])))
);


--
-- Name: TABLE trading_sqx_bridge_devices; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: trading_sqx_bridge_events; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.trading_sqx_bridge_events (
    id bigint NOT NULL,
    device_id uuid,
    command_id uuid,
    event_type text NOT NULL,
...
--
-- Name: TABLE trading_sqx_bridge_events; Type: COMMENT; Schema: public; Owner: -
--
...
--
-- Name: trading_sqx_bridge_events_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--

ALTER TABLE public.trading_sqx_bridge_events ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME public.trading_sqx_bridge_events_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
--
-- Name: trading_sqx_bridge_pairing_codes; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.trading_sqx_bridge_pairing_codes (
    code_hash text NOT NULL,
    label text,
    expires_at timestamp with time zone NOT NULL,
...
--
-- Name: trading_sqx_director_requests; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.trading_sqx_director_requests (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    device_id uuid NOT NULL,
    command_type text NOT NULL,
    project text,
    ...
    bridge_command_id uuid,
    blocked_reason text,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT trading_sqx_director_requests_command_type_check CHECK ((command_type = ANY (ARRAY['cli_help'::text, 'list_projects'::text, 'list_databanks'::text, 'status_project'::text, 'list_symbols'::text, 'list_instruments'::text, 'list_timezones'::text, 'count_databank'::text, 'export_databank'::text, 'list_strategies'::text, 'get_strategy_stats'::text, 'save_project_config'::text, 'run_vibe_nq6_smoke'::text, 'run_vibe_asset_research'::text, 'vibe_diagnose'::text, 'stack_health'::text, 'run_project'::text, 'stop_project'::text, 'pause_project'::text, 'resume_project'::text, 'live_project_control'::text, 'load_project_config'::text, 'update_data'::text, 'add_instrument'::text, 'add_symbol'::text, 'create_databank'::text, 'copy_databank'::text, 'move_databank'::text, 'freeze_databank'::text, 'sync_vibe_runtime'::text, 'start_vibe'::text, 'stop_vibe'::text, 'restart_vibe'::text]))),
    CONSTRAINT trading_sqx_director_requests_state_check CHECK ((state = ANY (ARRAY['QUEUED'::text, 'BLOCKED'::text, 'DISPATCHED'::text, 'COMPLETED'::text, 'FAILED'::text])))
);


--
-- Name: trading_sqx_governance_policy; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.trading_sqx_governance_policy (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    device_id uuid NOT NULL,
    mode text DEFAULT 'READ_ONLY'::text NOT NULL,
    allowed_projects text[] DEFAULT '{}'::text[] NOT NULL,
    require_local_control boolean DEFAULT true NOT NULL,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT trading_sqx_governance_policy_mode_check CHECK ((mode = ANY (ARRAY['READ_ONLY'::text, 'RESEARCH_CONTROL'::text])))
);


--
-- Name: trading_sqx_rara_verifications; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.trading_sqx_rara_verifications (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    director_request_id uuid,
    bridge_command_id uuid NOT NULL,
    verdict text NOT NULL,
    reason text NOT NULL,
    evidence jsonb DEFAULT '{}'::jsonb NOT NULL,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT trading_sqx_rara_verifications_verdict_check CHECK ((verdict = ANY (ARRAY['PASS'::text, 'FAIL'::text])))
);
...
--
-- Name: director_browser_action_audit director_browser_action_audit_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.director_browser_action_audit
    ADD CONSTRAINT director_browser_action_audit_pkey PRIMARY KEY (id);
...
--
-- Name: director_context_facts director_context_facts_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.director_context_facts
    ADD CONSTRAINT director_context_facts_pkey PRIMARY KEY (id);
...
--
-- Name: trading_sqx_bridge_commands trading_sqx_bridge_commands_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.trading_sqx_bridge_commands
    ADD CONSTRAINT trading_sqx_bridge_commands_pkey PRIMARY KEY (id);


--
-- Name: trading_sqx_bridge_devices trading_sqx_bridge_devices_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.trading_sqx_bridge_devices
    ADD CONSTRAINT trading_sqx_bridge_devices_pkey PRIMARY KEY (id);


--
-- Name: trading_sqx_bridge_devices trading_sqx_bridge_devices_token_hash_key; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.trading_sqx_bridge_devices
    ADD CONSTRAINT trading_sqx_bridge_devices_token_hash_key UNIQUE (token_hash);


--
-- Name: trading_sqx_bridge_events trading_sqx_bridge_events_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.trading_sqx_bridge_events
    ADD CONSTRAINT trading_sqx_bridge_events_pkey PRIMARY KEY (id);


--
-- Name: trading_sqx_bridge_pairing_codes trading_sqx_bridge_pairing_codes_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.trading_sqx_bridge_pairing_codes
    ADD CONSTRAINT trading_sqx_bridge_pairing_codes_pkey PRIMARY KEY (code_hash);


--
-- Name: trading_sqx_director_requests trading_sqx_director_requests_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.trading_sqx_director_requests
    ADD CONSTRAINT trading_sqx_director_requests_pkey PRIMARY KEY (id);


--
-- Name: trading_sqx_governance_policy trading_sqx_governance_policy_device_id_key; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.trading_sqx_governance_policy
    ADD CONSTRAINT trading_sqx_governance_policy_device_id_key UNIQUE (device_id);


--
-- Name: trading_sqx_governance_policy trading_sqx_governance_policy_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.trading_sqx_governance_policy
    ADD CONSTRAINT trading_sqx_governance_policy_pkey PRIMARY KEY (id);


--
-- Name: trading_sqx_rara_verifications trading_sqx_rara_verifications_bridge_command_id_key; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.trading_sqx_rara_verifications
    ADD CONSTRAINT trading_sqx_rara_verifications_bridge_command_id_key UNIQUE (bridge_command_id);


--
-- Name: trading_sqx_rara_verifications trading_sqx_rara_verifications_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.trading_sqx_rara_verifications
    ADD CONSTRAINT trading_sqx_rara_verifications_pkey PRIMARY KEY (id);
...
--
-- Name: director_browser_action_audit_project_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX director_browser_action_audit_project_idx ON public.director_browser_action_audit USING btree (project_key, created_at DESC);


--
-- Name: director_context_facts_one_active; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX director_context_facts_one_active ON public.director_context_facts USING btree (project_key, fact_key) WHERE (status = 'active'::text);


--
-- Name: director_context_facts_project_observed_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX director_context_facts_project_observed_idx ON public.director_context_facts USING btree (project_key, observed_at DESC);
...
--
-- Name: trading_sqx_bridge_commands_device_state_created_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX trading_sqx_bridge_commands_device_state_created_idx ON public.trading_sqx_bridge_commands USING btree (device_id, state, created_at);
...
--
-- Name: trading_sqx_bridge_commands trg_trading_sqx_rara_verify; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_trading_sqx_rara_verify AFTER UPDATE OF state ON public.trading_sqx_bridge_commands FOR EACH ROW EXECUTE FUNCTION public.trading_sqx_rara_verify_command();
...
--
-- Name: director_context_facts director_context_facts_supersedes_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.director_context_facts
    ADD CONSTRAINT director_context_facts_supersedes_id_fkey FOREIGN KEY (supersedes_id) REFERENCES public.director_context_facts(id);
...
--
-- Name: trading_sqx_bridge_commands trading_sqx_bridge_commands_device_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.trading_sqx_bridge_commands
    ADD CONSTRAINT trading_sqx_bridge_commands_device_id_fkey FOREIGN KEY (device_id) REFERENCES public.trading_sqx_bridge_devices(id) ON DELETE CASCADE;


--
-- Name: trading_sqx_bridge_events trading_sqx_bridge_events_command_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.trading_sqx_bridge_events
    ADD CONSTRAINT trading_sqx_bridge_events_command_id_fkey FOREIGN KEY (command_id) REFERENCES public.trading_sqx_bridge_commands(id) ON DELETE SET NULL;


--
-- Name: trading_sqx_bridge_events trading_sqx_bridge_events_device_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.trading_sqx_bridge_events
    ADD CONSTRAINT trading_sqx_bridge_events_device_id_fkey FOREIGN KEY (device_id) REFERENCES public.trading_sqx_bridge_devices(id) ON DELETE SET NULL;


--
-- Name: trading_sqx_director_requests trading_sqx_director_requests_bridge_command_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.trading_sqx_director_requests
    ADD CONSTRAINT trading_sqx_director_requests_bridge_command_id_fkey FOREIGN KEY (bridge_command_id) REFERENCES public.trading_sqx_bridge_commands(id) ON DELETE SET NULL;


--
-- Name: trading_sqx_director_requests trading_sqx_director_requests_device_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.trading_sqx_director_requests
    ADD CONSTRAINT trading_sqx_director_requests_device_id_fkey FOREIGN KEY (device_id) REFERENCES public.trading_sqx_bridge_devices(id) ON DELETE CASCADE;


--
-- Name: trading_sqx_governance_policy trading_sqx_governance_policy_device_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.trading_sqx_governance_policy
    ADD CONSTRAINT trading_sqx_governance_policy_device_id_fkey FOREIGN KEY (device_id) REFERENCES public.trading_sqx_bridge_devices(id) ON DELETE CASCADE;


--
-- Name: trading_sqx_rara_verifications trading_sqx_rara_verifications_bridge_command_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.trading_sqx_rara_verifications
    ADD CONSTRAINT trading_sqx_rara_verifications_bridge_command_id_fkey FOREIGN KEY (bridge_command_id) REFERENCES public.trading_sqx_bridge_commands(id) ON DELETE CASCADE;


--
-- Name: trading_sqx_rara_verifications trading_sqx_rara_verifications_director_request_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.trading_sqx_rara_verifications
    ADD CONSTRAINT trading_sqx_rara_verifications_director_request_id_fkey FOREIGN KEY (director_request_id) REFERENCES public.trading_sqx_director_requests(id) ON DELETE CASCADE;
...
--
-- Name: contentflow_workflow_e2e_state; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.contentflow_workflow_e2e_state ENABLE ROW LEVEL SECURITY;
...
--
-- Name: director_browser_action_audit; Type: ROW SECURITY; Schema: public; Owner: -
--
...
--
-- Name: director_context_facts; Type: ROW SECURITY; Schema: public; Owner: -
--
...
--
-- Name: trading_sqx_bridge_commands; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.trading_sqx_bridge_commands ENABLE ROW LEVEL SECURITY;

--
-- Name: trading_sqx_bridge_devices; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.trading_sqx_bridge_devices ENABLE ROW LEVEL SECURITY;

--
-- Name: trading_sqx_bridge_events; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.trading_sqx_bridge_events ENABLE ROW LEVEL SECURITY;

--
-- Name: trading_sqx_bridge_pairing_codes; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.trading_sqx_bridge_pairing_codes ENABLE ROW LEVEL SECURITY;

--
-- Name: trading_sqx_director_requests; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.trading_sqx_director_requests ENABLE ROW LEVEL SECURITY;

--
-- Name: trading_sqx_governance_policy; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.trading_sqx_governance_policy ENABLE ROW LEVEL SECURITY;

--
-- Name: trading_sqx_rara_verifications; Type: ROW SECURITY; Schema: public; Owner: -
--
...
--
-- Name: FUNCTION director_browser_action_gate(p_project_key text, p_actor text, p_domain text, p_action text, p_request_payload jsonb); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.director_browser_action_gate(p_project_key text, p_actor text, p_domain text, p_action text, p_request_payload jsonb) FROM PUBLIC;
GRANT ALL ON FUNCTION public.director_browser_action_gate(p_project_key text, p_actor text, p_domain text, p_action text, p_request_payload jsonb) TO service_role;


--
-- Name: TABLE director_context_facts; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.director_context_facts TO anon;
GRANT ALL ON TABLE public.director_context_facts TO authenticated;
GRANT ALL ON TABLE public.director_context_facts TO service_role;


--
-- Name: FUNCTION director_publish_context_fact(p_project_key text, p_fact_key text, p_payload jsonb, p_source text, p_source_ref text, p_observed_at timestamp with time zone, p_expires_at timestamp with time zone, p_verified boolean); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.director_publish_context_fact(p_project_key text, p_fact_key text, p_payload jsonb, p_source text, p_source_ref text, p_observed_at timestamp with time zone, p_expires_at timestamp with time zone, p_verified boolean) FROM PUBLIC;
GRANT ALL ON FUNCTION public.director_publish_context_fact(p_project_key text, p_fact_key text, p_payload jsonb, p_source text, p_source_ref text, p_observed_at timestamp with time zone, p_expires_at timestamp with time zone, p_verified boolean) TO service_role;
...
--
-- Name: FUNCTION trading_sqx_director_dispatch(p_command_type text, p_project text); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.trading_sqx_director_dispatch(p_command_type text, p_project text) FROM PUBLIC;
GRANT ALL ON FUNCTION public.trading_sqx_director_dispatch(p_command_type text, p_project text) TO anon;
GRANT ALL ON FUNCTION public.trading_sqx_director_dispatch(p_command_type text, p_project text) TO authenticated;
GRANT ALL ON FUNCTION public.trading_sqx_director_dispatch(p_command_type text, p_project text) TO service_role;


--
-- Name: FUNCTION trading_sqx_director_dispatch_payload(p_command_type text, p_payload jsonb); Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON FUNCTION public.trading_sqx_director_dispatch_payload(p_command_type text, p_payload jsonb) TO anon;
GRANT ALL ON FUNCTION public.trading_sqx_director_dispatch_payload(p_command_type text, p_payload jsonb) TO authenticated;
GRANT ALL ON FUNCTION public.trading_sqx_director_dispatch_payload(p_command_type text, p_payload jsonb) TO service_role;


--
-- Name: FUNCTION trading_sqx_director_dispatch_v2(p_command_type text, p_project text, p_payload jsonb); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.trading_sqx_director_dispatch_v2(p_command_type text, p_project text, p_payload jsonb) FROM PUBLIC;
GRANT ALL ON FUNCTION public.trading_sqx_director_dispatch_v2(p_command_type text, p_project text, p_payload jsonb) TO service_role;


--
-- Name: FUNCTION trading_sqx_rara_verify_command(); Type: ACL; Schema: public; Owner: -
--

REVOKE ALL ON FUNCTION public.trading_sqx_rara_verify_command() FROM PUBLIC;
GRANT ALL ON FUNCTION public.trading_sqx_rara_verify_command() TO service_role;
...
--
-- Name: TABLE director_browser_action_audit; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.director_browser_action_audit TO anon;
GRANT ALL ON TABLE public.director_browser_action_audit TO authenticated;
GRANT ALL ON TABLE public.director_browser_action_audit TO service_role;
...
--
-- Name: SEQUENCE director_context_facts_id_seq; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON SEQUENCE public.director_context_facts_id_seq TO anon;
GRANT ALL ON SEQUENCE public.director_context_facts_id_seq TO authenticated;
GRANT ALL ON SEQUENCE public.director_context_facts_id_seq TO service_role;
...
--
-- Name: TABLE trading_sqx_bridge_commands; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.trading_sqx_bridge_commands TO anon;
GRANT ALL ON TABLE public.trading_sqx_bridge_commands TO authenticated;
GRANT ALL ON TABLE public.trading_sqx_bridge_commands TO service_role;


--
-- Name: TABLE trading_sqx_bridge_devices; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.trading_sqx_bridge_devices TO anon;
GRANT ALL ON TABLE public.trading_sqx_bridge_devices TO authenticated;
GRANT ALL ON TABLE public.trading_sqx_bridge_devices TO service_role;


--
-- Name: TABLE trading_sqx_bridge_events; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.trading_sqx_bridge_events TO anon;
GRANT ALL ON TABLE public.trading_sqx_bridge_events TO authenticated;
GRANT ALL ON TABLE public.trading_sqx_bridge_events TO service_role;


--
-- Name: SEQUENCE trading_sqx_bridge_events_id_seq; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON SEQUENCE public.trading_sqx_bridge_events_id_seq TO anon;
GRANT ALL ON SEQUENCE public.trading_sqx_bridge_events_id_seq TO authenticated;
GRANT ALL ON SEQUENCE public.trading_sqx_bridge_events_id_seq TO service_role;


--
-- Name: TABLE trading_sqx_bridge_pairing_codes; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.trading_sqx_bridge_pairing_codes TO anon;
GRANT ALL ON TABLE public.trading_sqx_bridge_pairing_codes TO authenticated;
GRANT ALL ON TABLE public.trading_sqx_bridge_pairing_codes TO service_role;


--
-- Name: TABLE trading_sqx_director_requests; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.trading_sqx_director_requests TO anon;
GRANT ALL ON TABLE public.trading_sqx_director_requests TO authenticated;
GRANT ALL ON TABLE public.trading_sqx_director_requests TO service_role;


--
-- Name: TABLE trading_sqx_governance_policy; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.trading_sqx_governance_policy TO anon;
GRANT ALL ON TABLE public.trading_sqx_governance_policy TO authenticated;
GRANT ALL ON TABLE public.trading_sqx_governance_policy TO service_role;


--
-- Name: TABLE trading_sqx_rara_verifications; Type: ACL; Schema: public; Owner: -
--

GRANT ALL ON TABLE public.trading_sqx_rara_verifications TO anon;
GRANT ALL ON TABLE public.trading_sqx_rara_verifications TO authenticated;
GRANT ALL ON TABLE public.trading_sqx_rara_verifications TO service_role;
...
\unrestrict ssXRnYLJpo2IZm5GDEaTGlApEDvR8mDqLW8D4pxX80znzIhhTptvIYWH1WTpwQg
