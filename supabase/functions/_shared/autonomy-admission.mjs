...
export function evaluateAutonomyAdmission(input={}) {
  ...
  const receiptOnlyStale=receiptValidation.errors.length===1&&receiptValidation.errors[0]==='receipt_stale';
  const recoverySnapshotFresh=input.freshRecoverySnapshot===true;
  const recoveryAdmitted=receiptValidation.valid||(receiptOnlyStale&&recoverySnapshotFresh);
  ...
  const dwellSatisfied=recoveryAdmitted&&healthyDwellMs>=minimumDwellMs;
  ...
  if(!recoveryAdmitted)blockers.push('recovery_evidence_invalid');
  if(recoveryAdmitted&&!dwellSatisfied)blockers.push('stability_dwell_pending');
  ...
  return {
    ...
    blockers,
    signals:{telemetryHealthy,recoveryReceiptValid:receiptValidation.valid,recoveryReceiptErrors:receiptValidation.errors,recoveryReceiptAgeMs:receiptValidation.ageMs,recoverySnapshotFresh,recoveryAdmitted,healthyDwellMs,minimumDwellMs,dwellSatisfied,providerBudgetAdmitted:budgetAdmission.admitted,providerBudget:budgetAdmission,ownershipConflicts,openIncidents,openCircuits,retryStates,retryOpenRate,waitingForEvidence}
  };
}
