...
function Step([string]$Message){ Write-Host "[Cygnus SQX Bridge V6.1 Live Telemetry] $Message" }
...
Step "Replacing Bridge only. Existing SQX/GOLD process is intentionally left untouched."
...
if($proc.HasExited){ throw "BRIDGE_V61_EXITED_EARLY:rc=$($proc.ExitCode)" }

Write-Host ""
Write-Host "CYGNUS_SQX_BRIDGE_V61_LIVE_TELEMETRY=PASS"
Write-Host "PAIRING_PRESERVED=true"
Write-Host "HTTP_API=http://127.0.0.1:5050/call"
Write-Host "READ_ONLY_ATTACH=true"
