"""Discovery-only SHORT-only backtest for BES v2.

Changes from v1:
- SHORT only.
- No prerequisite to sweep/break above a prior structural high.
- The 3-candle exit means three consecutive bearish (red) candles; a green/doji
  candle resets the count.

All entry decisions use fully closed bars and execute at the next bar open.
Only 2025-09-25..2026-09-11 is used. Confirmatory and sealed holdout windows
remain untouched.
"""
...
OUT = Path("bollinger_ema_sar_v2_short_only_output")
COST_R = 0.03
...
def candle_patterns(g: pd.DataFrame):
    ...
    engulf_short = (
        (c < o) &
        (this_lo_body <= prev_lo_body) &
        (this_hi_body >= prev_hi_body)
    )
    return rocket_short.fillna(False), engulf_short.fillna(False)


def signal_masks(g: pd.DataFrame):
    rocket_short, engulf_short = candle_patterns(g)
    parts = {
        "bollinger_touch": (
            (g["high"].shift(1) >= g["bb_upper"].shift(1)) |
            (g["high"] >= g["bb_upper"])
        ),
        "ema20_slope": (
            (g["ema20"] < g["ema20"].shift(1)) &
            (g["ema20"].shift(1) < g["ema20"].shift(2)) &
            (g["ema20"].shift(2) < g["ema20"].shift(3))
        ),
        "ema3_9_cross": (
            (g["ema3"].shift(1) >= g["ema9"].shift(1)) &
            (g["ema3"] < g["ema9"])
        ),
        "close_vs_ema9": g["close"] < g["ema9"],
        "psar": g["psar"] > g["high"],
        "signal_shape": g["high"] < g["high"].shift(1),
        "pattern": rocket_short | engulf_short,
    }
    mask = pd.Series(True, index=g.index)
    for v in parts.values():
        mask &= v.fillna(False)
    return parts, mask, (rocket_short, engulf_short)


def pattern_name(i: int, pats) -> str:
    rocket_short, engulf_short = pats
    rocket = bool(rocket_short.iloc[i])
    engulf = bool(engulf_short.iloc[i])
    if rocket and engulf:
        return "rocket+engulfing"
    if rocket:
        return "rocket"
    return "engulfing"
...
def run_trade(g: pd.DataFrame, signal_i: int):
    entry_i = signal_i + 1
    if entry_i >= len(g):
        return None
    ...
    for j in range(entry_i, len(g)):
        ...
        # Conservative OHLC ambiguity: stop first if both are reachable in one bar.
        if stop_hit:
        ...
        bar_open = float(g.at[j, "open"])
        bar_close = float(g.at[j, "close"])
        if bar_close < bar_open:
            consecutive_red += 1
        else:
            consecutive_red = 0
        if consecutive_red >= 3:
            exit_i, exit_price, exit_reason = j, bar_close, "three_consecutive_red"
            break
...
def metrics(rows):
    ...
    return {
        ...
        "net_win_rate": float(sum(x > 0 for x in vals) / len(vals)) if vals else 0.0,
        ...
        "three_red_exit_rate": float(sum(r["exit_reason"] == "three_consecutive_red" for r in rows) / len(rows)) if rows else 0.0,
        "stop_hit_rate": float(sum(r["exit_reason"] == "stop" for r in rows) / len(rows)) if rows else 0.0,
        "median_bars_held": float(np.median([r["bars_held"] for r in rows])) if rows else 0.0,
    }
...
def main():
    ...
    parts, mask, pats = signal_masks(g)
    ...
    summary = {
        "strategy": "bollinger_ema_sar_v2_short_only",
        "symbol": symbol,
        ...
        "changes": [
            "SHORT_ONLY",
            "NO_STRUCTURAL_HIGH_SWEEP_GATE",
            "THREE_CONSECUTIVE_RED_CANDLE_EXIT"
        ],
        "overall": metrics(trades),
        "by_session": {},
        "by_pattern": {},
        "exit_reasons": dict(Counter(r["exit_reason"] for r in trades)),
        "filter_funnel": cumulative_funnel(parts),
    }
    ...
    (OUT / f"{symbol.replace('.', '_')}_summary.json").write_text(
