...
# Canonical futures data are ingested once from Yahoo outside the swarm and then
# frozen as local CSVs with checksums. All workers/backtests consume the same immutable
# local evidence; no per-agent network refetch is allowed inside a research run.
...
def _futures_grounding_contract() -> dict[str, Any]:
    """Verify normal futures parsing and Cygnus canonical-data grounding isolation."""
    ...
    canonical = grounding.extract_symbols_from_user_vars({
        "target": "ES=F",
        "market": "E-mini S&P 500 futures, CME",
        "tv_policy": "MUST use ONLY the frozen local dataset",
        "data_contract": "{\"identity_target\":\"ES=F\",\"data_source\":\"local\"}",
    })
    ok = (
        observed == expected
        and mixed
        and mixed[0] == "ES=F"
        and "ES.US" not in mixed
        and canonical == []
    )
    return {
        "ok": bool(ok),
        ...
        "observed": observed,
        "mixed_es": mixed,
        "canonical_run_grounding_symbols": canonical,
        "canonical_network_grounding_disabled": canonical == [],
...
def _futures_backtest_source_contract(asset: str = "ES") -> dict[str, Any]:
    ...
    for interval, meta in manifest["timeframes"].items():
        ...
        try:
            ...
            probes[interval] = {"ok": False, "symbol": symbol, "error": f"{type(exc).__name__}: {exc}"}
    ...
    return {
        ...
        "probes": probes,
        "invariant": (
            "Yahoo is used only for controlled ingestion. Every swarm worker and backtest "
            "must consume the exact local canonical dataset aliases/checksums; no fresh "
            "per-agent network market-data window."
        ),
    }
...
async def full_health(mcp_url: str) -> dict[str, Any]:
    ...
    try:
        async with Client(mcp_url) as client:
            ...
            local_symbols = ["CL_CYGNUS_5M", "ES_CYGNUS_5M", "NQ_CYGNUS_5M", "GC_CYGNUS_5M"]
            ...
            evidence["checks"]["market_data"] = {
                "ok": (
                    ...
                    and all(v == "local" for v in futures_sources.values())
                ),
                "rows": rows,
                "futures_sources": futures_sources,
                "futures_call": futures_data,
                "dxy_call": dxy_data,
                "invariant": "futures research runtime reads frozen local canonical datasets; Yahoo is ingestion-only",
            }
