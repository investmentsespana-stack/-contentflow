...
except ImportError:  # direct-script compatibility for research workflows
    from robustness_engine import max_drawdown_r
...
class CoalitionBacktester:
    ...
    def returns_for(self, events: Iterable[BacktestEvent], members: Sequence[str], direction: int, *, context: ContextKey | None = None) -> List[float]:
    ...
    def _metrics_from_returns(members: Sequence[str], direction: int, returns_r: Sequence[float]) -> CoalitionMetrics:
        ...
        if gross_loss > 0:
            ...
            profit_factor = 0.0
        return CoalitionMetrics(tuple(members), direction, trades, wins, win_rate, expectancy, profit_factor, max_drawdown_r(returns_r), total_r, 0.0, 0.0)

    def evaluate(self, events: Sequence[BacktestEvent], members: Sequence[str], direction: int, *, context: ContextKey | None = None) -> CoalitionMetrics:
        ...
        return CoalitionMetrics(base.members, base.direction, base.trades, base.wins, base.win_rate, base.expectancy_r, base.profit_factor, base.max_drawdown_r, base.total_r, best_member, base.win_rate - best_member)

    def search(self, events: Sequence[BacktestEvent], strategy_names: Sequence[str], direction: int, *, member_sizes: Sequence[int] = (2, 3, 4), min_trades: int = 20, context: ContextKey | None = None) -> List[CoalitionMetrics]:
        ...
        for size in member_sizes:
            ...
            for members in combinations(unique_names, size):
                ...
                if metrics.trades < min_trades or metrics.expectancy_r <= 0:
                    continue
        ...
        def rank_key(item: CoalitionMetrics) -> Tuple[float, float, float, float, int]:
            ...
            return item.win_rate, item.expectancy_r, pf, -item.max_drawdown_r, item.trades
        return sorted(results, key=rank_key, reverse=True)

    def search_long_short(self, events: Sequence[BacktestEvent], strategy_names: Sequence[str], *, member_sizes: Sequence[int] = (2, 3, 4), min_trades: int = 20, context: ContextKey | None = None) -> Dict[int, List[CoalitionMetrics]]:
        return {
            1: self.search(events, strategy_names, 1, member_sizes=member_sizes, min_trades=min_trades, context=context),
            -1: self.search(events, strategy_names, -1, member_sizes=member_sizes, min_trades=min_trades, context=context),
        }

    def train_oos_search(self, events: Sequence[BacktestEvent], strategy_names: Sequence[str], direction: int, *, train_fraction: float = 0.70, member_sizes: Sequence[int] = (2, 3, 4), min_train_trades: int = 20, min_oos_trades: int = 5, top_n_train: int = 25) -> List[Tuple[CoalitionMetrics, CoalitionMetrics]]:
        ...
        train = events[:split]
        oos = events[split:]
        train_ranked = self.search(train, strategy_names, direction, member_sizes=member_sizes, min_trades=min_train_trades)[:top_n_train]
        ...
        for train_metrics in train_ranked:
            ...
            if oos_metrics.trades < min_oos_trades or oos_metrics.expectancy_r <= 0:
                continue
            survivors.append((train_metrics, oos_metrics))
        return sorted(survivors, key=lambda pair: (pair[1].win_rate, pair[1].expectancy_r, -pair[1].max_drawdown_r), reverse=True)
