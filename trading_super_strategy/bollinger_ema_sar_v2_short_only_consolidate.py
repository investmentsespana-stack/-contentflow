"""Consolidate BES v2 SHORT-only discovery artifacts."""
...
ROOT = Path("bollinger_ema_sar_v2_short_only_downloads")
OUT = Path("bollinger_ema_sar_v2_short_only_crossmarket")
...
def metrics(df):
    ...
    return {
        ...
        "three_red_exit_rate": float((df["exit_reason"] == "three_consecutive_red").mean()),
...
def main():
    ...
    for p in ROOT.rglob("*_trades.csv"):
        try:
            ...
            pass
    cols = ["symbol", "direction", "session", "pattern", "net_r", "exit_reason", "bars_held", "entry_time_et"]
    ...
    result = {
        ...
        "changes": ["SHORT_ONLY", "NO_STRUCTURAL_HIGH_SWEEP_GATE", "THREE_CONSECUTIVE_RED_CANDLE_EXIT"],
