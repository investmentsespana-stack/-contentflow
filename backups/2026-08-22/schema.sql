...
\restrict GhbPXi0MCpnAzXf29jJ6zkHR6mY6CIzhq2Z4fmX1eWnkhIzrhqsSUjHeqv0jnvD
...
begin
 if old.status is distinct from new.status and new.status='running' and new.project_key='contentflow' then
...
begin
  ...
  if not found then
    update public.contentflow_tool_execution_queue
       set state='blocked',claim_token=null,claimed_at=null,updated_at=now(),last_error='BACKLOG_CLAIM_FAILED'
     where id=qid and claim_token=tok;
    return;
  end if;
...
begin
  if new.project_key = 'contentflow'
     ...
     and old.status is distinct from 'running' then
    perform pg_advisory_xact_lock(hashtext('contentflow:master-running-cap'));
    if (select count(*) from public.contentflow_build_backlog b where b.project_key='contentflow' and b.status='running' and b.id <> new.id) >= 2 then
...
CREATE FUNCTION public.contentflow_normalize_dispatchability(p_project_key text DEFAULT 'contentflow'::text) RETURNS jsonb
    ...
    SET search_path TO 'public'
    AS $$
declare to_ready int:=0; to_planned int:=0; retry_state jsonb; begin
 ...
 return jsonb_build_object('to_ready',to_ready,'to_planned',to_planned,'retry_policy',retry_state);
end $$;
...
CREATE FUNCTION public.contentflow_record_runtime_verification(p_builder_run_id bigint, p_verification_type text, p_passed boolean, p_evidence jsonb DEFAULT '{}'::jsonb, p_verifier text DEFAULT 'runtime_verifier'::text) RETURNS jsonb
    ...
    SET search_path TO 'public'
    AS $$
declare r public.contentflow_builder_runs%rowtype; b public.contentflow_build_backlog%rowtype; promoted boolean:=false;
begin
  if coalesce(auth.role(),'') <> 'service_role' then raise exception 'service_role_required'; end if;
  ...
  if p_passed then
    update public.contentflow_build_backlog set runtime_verified=true,runtime_verified_at=now(),runtime_evidence=coalesce(p_evidence,'{}'::jsonb),completion_phase='runtime_proven',updated_at=now() where id=b.id;
  ...
  values(b.project_key,'runtime_verification_recorded',b.task_key,'runtime_verifier','completion_boundary',case when p_passed then case when promoted then 'pass_promoted_completed' else 'pass_waiting_review' end else 'fail' end,false,format('run=%s type=%s verifier=%s',r.id,coalesce(p_verification_type,'runtime_check'),coalesce(p_verifier,'runtime_verifier')),now());

  return jsonb_build_object('ok',true,'task_key',b.task_key,'passed',p_passed,'review_approved',coalesce(r.review_approved,false),'promoted_completed',promoted,'completion_phase',case when p_passed then 'runtime_proven' else 'verification_required' end);
end $$;
...
CREATE FUNCTION public.contentflow_requires_runtime_evidence(p_task_type text, p_title text, p_description text, p_acceptance_criteria text) RETURNS boolean
    ...
    SET search_path TO 'public'
    AS $$
  select coalesce(p_task_type,'') in ('code','architecture')
    or lower(coalesce(p_title,'')||' '||coalesce(p_description,'')||' '||coalesce(p_acceptance_criteria,'')) ~
       '(runtime|deploy|deployment|integration|endpoint|database|sql|migration|guardrail|rollback|boundary|enforce|blocking|block |restore|recovery|trigger|policy|rls|secret|vault|cron|worker|dispatcher|lease|claim|idempot|api)';
$$;
...
CREATE FUNCTION public.rara_apply_known_repairs(p_project_key text DEFAULT 'contentflow'::text, p_limit integer DEFAULT 10) RETURNS jsonb
    ...
    SET search_path TO 'public'
    AS $$
declare x record; a jsonb; n int:=0; fixed int:=0; escalated int:=0;
begin
  update public.director_repair_incidents
     set status='needs_help', requires_human=true, updated_at=now(),
         outcome=case when coalesce(outcome,'') like '%attempt_budget_exhausted%' then outcome else coalesce(outcome,'')||case when coalesce(outcome,'')='' then '' else '|' end||'attempt_budget_exhausted' end
   where project_key=p_project_key
     and status in ('open','analyzing','repairing','validating')
  ...
  loop
    a:=public.rara_apply_known_repair(x.id);
    n:=n+1;
    if coalesce((a->>'ok')::boolean,false) then fixed:=fixed+1; end if;
  end loop;
  return jsonb_build_object('examined',n,'repaired',fixed,'attempt_budget_escalated',escalated);
end $$;
...
CREATE FUNCTION public.rara_claim_next_incident(p_project_key text DEFAULT 'contentflow'::text) RETURNS TABLE(id bigint)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$ begin return query with c as (select i.id from public.director_repair_incidents i where i.project_key=p_project_key and i.status='open' order by i.created_at asc for update skip locked limit 1) update public.director_repair_incidents x set status='analyzing',attempts=x.attempts+1,updated_at=now() from c where x.id=c.id returning x.id; end $$;
...
\unrestrict GhbPXi0MCpnAzXf29jJ6zkHR6mY6CIzhq2Z4fmX1eWnkhIzrhqsSUjHeqv0jnvD
