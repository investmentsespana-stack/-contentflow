"""Discovery-only slope-refinement backtest for the preregistered 1m Bollinger/EMA/SAR setup.

This script changes ONLY the sideways/slope filter versus v1. It uses a 3-bar net EMA20 slope normalized by ATR14 (>= 0.05 ATR in the trade direction), allowing small pullbacks inside an otherwise sloped segment. It intentionally uses only 2025-09-25..2026-09-11 and does not touch
confirmatory or sealed holdout windows. All entry decisions are based on fully
closed bars; execution is at the next bar open.
"""
...
def signal_masks(g: pd.DataFrame):
    ...
    long_parts = {
        ...
        "ema20_slope": (g["ema20"] - g["ema20"].shift(3)) >= (0.05 * g["atr14"]),
...
def main():
    ...
    summary = {
        "strategy": "bollinger_ema_sar_v1_1_slope",
        "symbol": symbol,
