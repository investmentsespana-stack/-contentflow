...
declare
  ...
  v_project_required constant text[] := array[
    ...
    'freeze_databank','live_project_control'
  ];
  ...
  v_live_action text := lower(trim(coalesce(p_payload->>'action','')));
begin
  ...
  elsif p_command_type='live_project_control' and v_live_action not in ('start','stop','pause','resume') then
    v_reason := 'live_control_action_not_approved';
