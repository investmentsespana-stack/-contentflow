...
function Step([string]$Message){ Write-Host "[Cygnus SQX Bridge V6.3 Live Control] $Message" }
...
Step "Replacing Bridge only. StrategyQuant and GOLD are not stopped by this installer."
...
if($proc.HasExited){ throw "BRIDGE_V63_EXITED_EARLY:rc=$($proc.ExitCode)" }

Write-Host ""
Write-Host "CYGNUS_SQX_BRIDGE_V63_LIVE_CONTROL=PASS"
Write-Host "PAIRING_PRESERVED=true"
Write-Host "HTTP_GUI_CONTROL=true"
Write-Host "DATABANK_FREEZE=true"
Write-Host "SEMANTIC_CLI_FAILURE_GATE=true"
Write-Host "SQX_PROCESS_TOUCHED=false"
Write-Host "GOLD_TOUCHED=false"
Write-Host "QUEUED_REMOTE_COMMANDS_WILL_EXECUTE=true"
