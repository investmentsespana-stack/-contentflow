...
\restrict XKSalhwRdw12I0jGPdE0p1ZWoQVelhpxxV15iqYzNH4p1ig4etxpxJJxdTdUouS
...
begin
 ...
 if new.status='ready' then
   if exists(select 1 from public.contentflow_builder_runs r where r.backlog_task_id=new.id and r.status='review_required') then
     ...
     if not durable then update public.contentflow_retry_state set circuit_state='closed',circuit_open_until=null,next_retry_at=null,updated_at=now() where project_key=new.project_key and task_key=new.task_key and circuit_state='open'; end if;
   end if;
 elsif new.status='blocked' then
   if coalesce(new.blocked_reason,'')='REVIEW_PENDING' or exists(select 1 from public.contentflow_builder_runs r where r.backlog_task_id=new.id and r.status='review_required') then
...
begin
 ...
 where er.backlog_task_id=b.id and b.project_key=p_project_key
   ...
   and coalesce(b.workflow_contract->>'evidence_policy','declared_gaps_allowed')<>'required'
   and er.status in ('open','task_created');
 get diagnostics obsolete_evidence=row_count;

 for rec in
   select b.id,b.project_key,b.depends_on
   from public.contentflow_build_backlog b
   where b.project_key=p_project_key
     and (coalesce(b.workflow_contract,'{}'::jsonb) ? 'runtime_required' or coalesce(b.workflow_contract->>'contract_version','')<>'')
     and coalesce(b.workflow_contract->>'evidence_policy','declared_gaps_allowed')<>'required'
 ...
 set status='deferred',blocked_reason='OBSOLETE_BY_DURABLE_CONTRACT_V2',updated_at=now()
 ...
 update public.director_repair_incidents i
 set status='resolved',requires_human=false,resolved_at=now(),updated_at=now(),outcome='superseded_by_durable_contract_v2',diagnosis='Legacy escalation contradicted explicit durable workflow contract',validation='contract_driven_reconciliation'
...
begin
 ...
 v_retryable:=v_failure='INFRA_FAIL' or p_http_status in (408,425,429,500,502,503,504) or lower(v_reason) ~ '(transport|timeout|capacity|rate.limit|upstream|network|fenced)';
...
begin
  ...
  -- Evidence activities keep the existing evidence-capability contract.
  ...
  -- Direct tool work is a different contract. Never let the evidence runner
  -- claim it unless the task declares a deterministic execution recipe.
...
begin
 ...
 -- Async executor v3 returns HTTP 202 immediately. This is an acknowledgement, not the task result.
 if coalesce((v_j->>'accepted')::boolean,false)=true and coalesce(v_j->>'architecture','')='DURABLE_TASK_STATE_MACHINE_V3_ASYNC_EXECUTOR' then
   ...
   insert into public.contentflow_runtime_event_ledger(builder_run_id,task_key,event_type,idempotency_key,actor,payload)
   values(v_run.id,v_run.task_key,'async_dispatch_ack_collected',v_run.idempotency_key,'collector',jsonb_build_object('request_id',p_request_id,'http_status',v_r.status_code)) on conflict do nothing;
...
CREATE FUNCTION public.rara_apply_review_decision_v2(p_builder_run_id bigint, p_claim_token text, p_approve boolean, p_reason text DEFAULT NULL::text) RETURNS jsonb
    ...
    SET search_path TO 'public'
    AS $$
declare q public.contentflow_review_work_queue%rowtype; d jsonb; l jsonb; b public.contentflow_build_backlog%rowtype;
begin
 if coalesce(auth.role(),'')<>'service_role' then raise exception 'service_role_required'; end if;
 ...
 if coalesce((d->>'ok')::boolean,false)=false and p_approve and d->>'reason' in ('deterministic_evidence_gate_failed','deterministic_stage_gate_failed') then d:=public.rara_apply_review_decision(p_builder_run_id,false,'Deterministic stage gate failed at commit: '||coalesce(p_reason,'')); end if;
 if coalesce((d->>'ok')::boolean,false)=false then update public.contentflow_review_work_queue set state='pending',claim_token=null,claimed_at=null,available_at=now()+interval '60 seconds',last_error=left(coalesce(d->>'reason','decision_failed'),1000),updated_at=now() where builder_run_id=p_builder_run_id and state='claimed' and claim_token=p_claim_token; return d; end if;
 ...
 if coalesce(d->>'decision','') in ('rejected_patch_required','rejected_requeued') then
   l:=public.rara_learn_and_replan_rejection(p_builder_run_id,coalesce(p_reason,'')); d:=d||jsonb_build_object('learning',l);
...
\unrestrict XKSalhwRdw12I0jGPdE0p1ZWoQVelhpxxV15iqYzNH4p1ig4etxpxJJxdTdUouS
