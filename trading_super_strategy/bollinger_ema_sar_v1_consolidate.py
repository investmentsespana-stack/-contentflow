"""Consolidate per-market Bollinger/EMA/SAR v1 discovery artifacts."""
...
ROOT = Path("bollinger_ema_sar_v1_downloads")
OUT = Path("bollinger_ema_sar_v1_crossmarket")
...
def main():
    ...
    for p in files:
        try:
            ...
            pass
    all_df = pd.concat(frames, ignore_index=True) if frames else pd.DataFrame(columns=["symbol","direction","session","pattern","net_r","exit_reason"])
    if not all_df.empty:
        all_df = all_df.sort_values("entry_time_et").reset_index(drop=True)
    all_df.to_csv(OUT / "all_trades.csv", index=False)

    result = {
        "strategy": "bollinger_ema_sar_v1",
