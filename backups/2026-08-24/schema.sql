...
\restrict nizyeI3wwMtX74YBWUwCNk6hkvvAk1s00KbCKIFjuf6JWbJ4vU1cYfk9jmr1RyN
...
CREATE FUNCTION public.contentflow_activity_begin_v1(p_run_id bigint, p_lease_token text, p_runner_instance_id text, p_phase text, p_timeout_seconds integer) RETURNS jsonb
    ...
    SET search_path TO 'public'
    AS $$
declare r public.contentflow_builder_runs%rowtype; v_deadline timestamptz;
begin
 ...
 v_deadline:=now()+make_interval(secs=>p_timeout_seconds+20);
 update public.contentflow_builder_runs set activity_phase=p_phase,activity_deadline_at=v_deadline,heartbeat_deadline_at=coalesce(heartbeat_deadline_at,now()+interval '75 seconds'),activity_seq=activity_seq+1,heartbeat_at=now(),runner_instance_id=coalesce(runner_instance_id,p_runner_instance_id),status=case when status='claimed' then 'running' else status end,activity_checkpoint=jsonb_set(coalesce(activity_checkpoint,'{}'::jsonb),'{current_phase}',to_jsonb(p_phase),true) where id=p_run_id;
 insert into public.contentflow_runtime_event_ledger(builder_run_id,task_key,event_type,idempotency_key,actor,payload)
 values(p_run_id,r.task_key,'activity_started',r.idempotency_key,'builder_runner_v4',jsonb_build_object('phase',p_phase,'deadline_at',v_deadline,'runner_instance_id',p_runner_instance_id,'seq',r.activity_seq+1)) on conflict do nothing;
 return jsonb_build_object('ok',true,'phase',p_phase,'deadline_at',v_deadline);
end $$;
...
begin
 ...
 update public.contentflow_builder_runs set activity_phase=null,activity_deadline_at=null,heartbeat_at=now(),heartbeat_deadline_at=now()+interval '75 seconds',activity_checkpoint=v_checkpoint where id=p_run_id;
 insert into public.contentflow_runtime_event_ledger(builder_run_id,task_key,event_type,idempotency_key,actor,payload)
...
begin
 ...
 where q.project_key=p_project_key and q.state='pending' and q.attempts<12
   and b.status in ('blocked','ready') and b.execution_lane in ('tool_executor','evidence_producer')
 ...
 update public.contentflow_build_backlog set status='running',blocked_reason=null,updated_at=now() where id=bid and status in ('blocked','ready') and execution_lane in ('tool_executor','evidence_producer');
...
CREATE FUNCTION public.contentflow_classify_run_error(p_error text) RETURNS text
 ...
 select case
   ...
   when coalesce(p_error,'') ilike '%worker_provider_failed%' or coalesce(p_error,'') ilike '%provider_failed%' or coalesce(p_error,'') ilike '%runner_response_parse_failed%' or coalesce(p_error,'') ilike '%runner_response_parse%' then 'provider'
...
CREATE FUNCTION public.contentflow_clear_retry_after_repair(p_task_key text) RETURNS boolean
    ...
    SET search_path TO 'public'
    AS $$ begin
 update public.contentflow_retry_state set attempt_count=0,next_retry_at=null,circuit_state='closed',circuit_open_until=null,updated_at=now() where project_key='contentflow' and task_key=p_task_key;
 update public.contentflow_build_backlog set next_eligible_at=now(),updated_at=now() where project_key='contentflow' and task_key=p_task_key and status in ('ready','blocked','planned');
 return found;
end $$;
...
CREATE FUNCTION public.contentflow_clear_retry_after_repair(p_task_key text, p_project_key text DEFAULT 'contentflow'::text) RETURNS boolean
    ...
    AS $$
begin
 update public.contentflow_retry_state set attempt_count=0,next_retry_at=null,circuit_state='closed',circuit_open_until=null,updated_at=now() where project_key=p_project_key and task_key=p_task_key;
 update public.contentflow_build_backlog set next_eligible_at=now(),blocked_reason=null,updated_at=now() where project_key=p_project_key and task_key=p_task_key and status in ('ready','blocked','planned');
 return found;
end $$;
...
CREATE FUNCTION public.contentflow_dependency_release_reconcile(p_project_key text DEFAULT 'contentflow'::text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$ declare released int:=0; held_deps int:=0; held_circuit int:=0; held_capability int:=0; begin update public.contentflow_build_backlog b set status='ready',updated_at=now() where b.project_key=p_project_key and b.status='blocked' and (b.next_eligible_at is null or b.next_eligible_at<=now()) and not exists(select 1 from public.contentflow_retry_state rs where rs.backlog_task_id=b.id and rs.circuit_state='open') and not exists(select 1 from jsonb_array_elements_text(coalesce(b.depends_on,'[]'::jsonb)) dep(value) where not exists(select 1 from public.contentflow_build_backlog d where d.project_key=b.project_key and d.task_key=dep.value and d.status='completed')) and (coalesce(b.execution_lane,'llm_artifact') not in ('tool_executor','evidence_producer') or public.contentflow_tool_execution_capability_ready(p_project_key,b.task_key)); get diagnostics released=row_count; select count(*) into held_deps from public.contentflow_build_backlog b where b.project_key=p_project_key and b.status='blocked' and exists(select 1 from jsonb_array_elements_text(coalesce(b.depends_on,'[]'::jsonb)) dep(value) where not exists(select 1 from public.contentflow_build_backlog d where d.project_key=b.project_key and d.task_key=dep.value and d.status='completed')); select count(*) into held_circuit from public.contentflow_build_backlog b where b.project_key=p_project_key and b.status='blocked' and exists(select 1 from public.contentflow_retry_state rs where rs.backlog_task_id=b.id and rs.circuit_state='open'); select count(*) into held_capability from public.contentflow_build_backlog b where b.project_key=p_project_key and b.status='blocked' and coalesce(b.execution_lane,'llm_artifact') in ('tool_executor','evidence_producer') and not public.contentflow_tool_execution_capability_ready(p_project_key,b.task_key); perform public.contentflow_sync_tool_execution_queue(p_project_key); return jsonb_build_object('architecture','DEPENDENCY_RELEASE_RECONCILIATION_V1','released',released,'held_by_dependencies',held_deps,'held_by_circuit',held_circuit,'held_by_capability',held_capability); end $$;
...
CREATE FUNCTION public.contentflow_finalize_run_v2(p_run_id bigint, p_lease_token text, p_http_status integer, p_payload jsonb) RETURNS jsonb
    ...
    SET search_path TO 'public'
    AS $$
declare r public.contentflow_builder_runs%rowtype; b public.contentflow_build_backlog%rowtype; v_ok boolean; v_quality numeric; v_cost numeric; v_model text; v_result text; v_failure text; v_final text; v_reason text; v_dep_incomplete int:=0;
begin
 if coalesce(auth.role(),'')<>'service_role' then raise exception 'service_role_required'; end if;
 select * into r from public.contentflow_builder_runs where id=p_run_id for update;
 if not found then return jsonb_build_object('ok',false,'reason','run_not_found'); end if;
 select * into b from public.contentflow_build_backlog where id=r.backlog_task_id for update;
 if not found then return jsonb_build_object('ok',false,'reason','backlog_not_found'); end if;
 if r.lease_token is distinct from p_lease_token or r.lease_revoked_at is not null or r.status not in ('claimed','running') or b.status<>'running' or exists(select 1 from public.contentflow_builder_runs n where n.backlog_task_id=r.backlog_task_id and n.id>r.id) then
   update public.contentflow_builder_dispatches set status='superseded',collected_at=now(),http_status=p_http_status,error='fenced_out_or_commit_state_invalid' where builder_run_id=r.id and status='pending';
   return jsonb_build_object('ok',false,'reason','commit_fenced','run_id',r.id,'lease_generation',r.lease_generation);
 end if;
 if b.depends_on is not null then
   select count(*) into v_dep_incomplete from jsonb_array_elements_text(coalesce(b.depends_on,'[]'::jsonb)) d(dep) where not exists(select 1 from public.contentflow_build_backlog x where x.project_key=b.project_key and x.task_key=d.dep and x.status='completed');
 end if;
 v_ok:=coalesce((p_payload->>'ok')::boolean,false); v_quality:=coalesce((p_payload->>'quality_score')::numeric,0); v_cost:=coalesce((p_payload->>'cost_usd')::numeric,0); v_model:=nullif(p_payload->>'selected_model',''); v_result:=nullif(p_payload->>'result',''); v_failure:=coalesce(p_payload->>'failure_class',''); v_reason:=coalesce(p_payload->>'quality_reason',p_payload->>'error','execution_failed');
 if v_cost<0 or v_cost>0.05 then v_ok:=false; v_reason:='cost_gate_failed'; end if;
 if v_dep_incomplete>0 then v_ok:=false; v_reason:='dependency_commit_gate_failed'; end if;
 if v_failure='JUDGE_FAIL' and v_result is not null then v_final:='review_required'; elsif v_ok and v_quality>=85 then v_final:='review_required'; else v_final:='failed'; end if;
 update public.contentflow_builder_runs set status=v_final,selected_model=coalesce(v_model,r.selected_model),quality_score=v_quality,cost_usd=least(greatest(v_cost,0),0.05),result=v_result,error=case when v_final='failed' then v_reason when v_final='review_required' and v_failure='JUDGE_FAIL' then v_reason else null end,review_approved=false,finished_at=case when v_final='failed' then now() else null end,lease_revoked_at=now(),activity_phase=null,activity_deadline_at=null,heartbeat_deadline_at=null where id=r.id;
 update public.contentflow_build_backlog set status=case when v_final='review_required' then 'blocked' else v_final end,selected_model=coalesce(v_model,r.selected_model),quality_score=v_quality,cost_usd=least(greatest(v_cost,0),0.05),result=v_result,updated_at=now() where id=r.backlog_task_id and status='running';
 update public.director_worker_queue set status='ready',current_task_key=null,last_task_key=r.task_key,last_outcome=v_final,last_quality_score=v_quality,last_finished_at=now(),total_completions=total_completions+case when v_final='review_required' then 1 else 0 end,total_failures=total_failures+case when v_final='failed' then 1 else 0 end,updated_at=now() where model_id=r.selected_model and current_task_key is not distinct from r.task_key;
 update public.contentflow_builder_dispatches set status='collected',collected_at=now(),http_status=p_http_status,error=case when v_final='failed' then v_reason else null end where builder_run_id=r.id and status='pending';
 insert into public.contentflow_runtime_event_ledger(builder_run_id,task_key,event_type,idempotency_key,actor,payload) values(r.id,r.task_key,'owner_finalized',r.idempotency_key,'dispatch_executor_v2',jsonb_build_object('final_status',v_final,'http_status',p_http_status,'quality_score',v_quality,'lease_generation',r.lease_generation,'dependency_commit_gate',v_dep_incomplete=0)) on conflict do nothing;
 return jsonb_build_object('ok',true,'run_id',r.id,'final_status',v_final,'quality_score',v_quality,'dependency_commit_gate',v_dep_incomplete=0);
end $$;
...
begin
  if new.status='completed'
     and coalesce(new.epic,'') not in ('evidence_capability','evidence_capability_root','capability_bootstrap')
  ...
  elsif new.status='completed' and coalesce(new.epic,'') in ('evidence_capability','evidence_capability_root','capability_bootstrap') then
...
begin
  if new.status='completed' then
    ...
    elsif found and coalesce(b.epic,'') in ('evidence_capability','evidence_capability_root','capability_bootstrap') then
      new.error:=null;
...
CREATE FUNCTION public.contentflow_master_reconcile(p_project_key text DEFAULT 'contentflow'::text) RETURNS jsonb
    ...
    AS $$
declare
 v_graph jsonb:='{}'::jsonb; v_orphans jsonb:='{}'::jsonb; v_state jsonb:='{}'::jsonb; v_reviews jsonb:='{}'::jsonb; v_retry jsonb:='{}'::jsonb; v_progress jsonb:='{}'::jsonb; v_known jsonb:='{}'::jsonb; v_slo jsonb:='{}'::jsonb; v_test jsonb:='{}'::jsonb; v_obsolete jsonb:='{}'::jsonb; v_help jsonb:='{}'::jsonb; v_tools jsonb:='{}'::jsonb; v_leases int:=0; v_detected int:=0;
begin
 if coalesce(auth.role(),'')<>'service_role' then raise exception 'service_role_required'; end if;
 ...
 begin v_known:=public.rara_apply_known_repairs(p_project_key,50); exception when others then v_known:=jsonb_build_object('error',sqlerrm); end;
 ...
 return jsonb_build_object('architecture','MASTER_DIRECTOR_CONTROL_PLANE_V3','dependency_graph',v_graph,'orphans',v_orphans,'runtime_state',v_state,'leases_recovered',v_leases,'review_gate',v_reviews,'retry',v_retry,'progress',v_progress,'tool_queue',v_tools,'incidents_detected',v_detected,'known_repairs',v_known,'help_escalation',v_help,'obsolete_resolved',v_obsolete,'slo',v_slo,'resilience',v_test);
end;
$$;
...
CREATE FUNCTION public.contentflow_normalize_dispatchability(p_project_key text DEFAULT 'contentflow'::text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$ declare to_ready int:=0; to_planned int:=0; to_blocked_tool int:=0; to_blocked_evidence int:=0; retry_state jsonb; cap_plan jsonb; begin
 ...
 update public.contentflow_build_backlog b set status='ready',updated_at=now()
 where b.project_key=p_project_key and b.status='planned'
   and (b.next_eligible_at is null or b.next_eligible_at<=now())
   and not exists(select 1 from public.contentflow_retry_state rs where rs.backlog_task_id=b.id and rs.circuit_state='open')
 ...
 get diagnostics to_ready=row_count;
 update public.contentflow_build_backlog b set status='planned',updated_at=now()
 where b.project_key=p_project_key and b.status='ready'
   and exists(select 1 from jsonb_array_elements_text(coalesce(b.depends_on,'[]'::jsonb)) dep(value) where not exists(select 1 from public.contentflow_build_backlog d where d.project_key=b.project_key and d.task_key=dep.value and d.status='completed'));
 get diagnostics to_planned=row_count;
 update public.contentflow_build_backlog b set status='blocked',updated_at=now()
 where b.project_key=p_project_key and b.status='ready' and b.execution_lane='tool_executor' and not public.contentflow_tool_execution_capability_ready(p_project_key,b.task_key);
 get diagnostics to_blocked_tool=row_count;
 update public.contentflow_build_backlog b set status='blocked',updated_at=now()
 where b.project_key=p_project_key and b.status='ready' and b.execution_lane='evidence_producer' and not public.contentflow_tool_execution_capability_ready(p_project_key,b.task_key);
 get diagnostics to_blocked_evidence=row_count;
 return jsonb_build_object('architecture','DISPATCHABILITY_INVARIANT_V3_CAPABILITY_FIRST','capability_plan',cap_plan,'to_ready',to_ready,'to_planned',to_planned,'to_blocked_tool',to_blocked_tool,'to_blocked_evidence',to_blocked_evidence,'retry_policy',retry_state);
end $$;
...
CREATE FUNCTION public.contentflow_plan_execution_buffer(p_project_key text DEFAULT 'contentflow'::text, p_target integer DEFAULT 10) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$ begin if p_project_key<>'contentflow' then return jsonb_build_object('ok',true,'architecture','INFRASTRUCTURE_PLANNER_SCOPE_GUARD_V1','scope','contentflow_only','project_key',p_project_key,'skipped',true); end if; return public.contentflow_plan_execution_buffer_internal_v1(p_project_key,p_target); end $$;
...
CREATE FUNCTION public.contentflow_progress_stall_reconcile(p_project_key text DEFAULT 'contentflow'::text) RETURNS jsonb
    ...
    SET search_path TO 'public'
    AS $$
declare v_ready int:=0; v_workers int:=0; v_running int:=0; v_before int:=0; v_after int:=0; v_retry_cleaned int:=0; v_dep_fixed int:=0; v_created int:=0;
begin
 if coalesce(auth.role(),'')<>'service_role' then raise exception 'service_role_required'; end if;
 select count(*) into v_ready from public.contentflow_build_backlog where project_key=p_project_key and status='ready';
 select count(*) into v_workers from public.director_worker_queue where status='ready';
 ...
 -- Repair only stale circuit state that has been superseded by an explicit later requeue/repair.
 ...
 where s.backlog_task_id=b.id and s.project_key=p_project_key and b.project_key=p_project_key
   and s.circuit_state='open' and b.status='ready' and b.updated_at>s.updated_at
 ...
 -- Canonicalize dependency arrays: remove self references and duplicates only; never remove a real external dependency.
 with fixed as (
 ...
 if v_ready>0 and v_workers>0 and v_running=0 and v_after=0 then
   ...
   select p_project_key,'director_control','progress_stall','progress_stall','Ready work and ready workers exist but no task is dispatchable and no worker is running',jsonb_build_object('ready_tasks',v_ready,'workers_ready',v_workers,'workers_running',v_running,'dispatchable_before',v_before,'dispatchable_after',v_after,'retry_state_cleaned',v_retry_cleaned,'dependency_rows_fixed',v_dep_fixed),'medium','open',3,false
   ...
   update public.director_repair_incidents set status='resolved',resolved_at=now(),updated_at=now(),outcome='progress_restored_by_reconciler',requires_human=false
   where project_key=p_project_key and error_fingerprint='progress_stall' and status in ('open','analyzing','repairing','validating');
 end if;

 insert into public.director_autonomy_events(project_key,event_type,source,assignment_mode,outcome,required_user_intervention,notes,finished_at)
 values(p_project_key,'progress_stall_reconcile','master_director_v3','desired_vs_actual',case when v_after>0 or v_ready=0 or v_workers=0 or v_running>0 then 'healthy_or_repaired' else 'stalled' end,false,jsonb_build_object('ready',v_ready,'workers_ready',v_workers,'running',v_running,'dispatchable_before',v_before,'dispatchable_after',v_after,'retry_cleaned',v_retry_cleaned,'deps_fixed',v_dep_fixed,'incident_created',v_created)::text,now());
 return jsonb_build_object('ready',v_ready,'workers_ready',v_workers,'running',v_running,'dispatchable_before',v_before,'dispatchable_after',v_after,'retry_cleaned',v_retry_cleaned,'deps_fixed',v_dep_fixed,'incident_created',v_created);
end $$;
...
begin
 ...
 set state='blocked',claim_token=null,claimed_at=null,updated_at=now(),
     last_error='CAPABILITY_WAIT:'||coalesce(m.prerequisite,'unknown')
 from public.contentflow_build_backlog b
 left join public.contentflow_evidence_requirements er on er.project_key=b.project_key and er.evidence_task_key=b.task_key
 ...
 return jsonb_build_object('architecture','EVIDENCE_CAPABILITY_ADMISSION_V1','held',held,'released',released,
   'pending',(select count(*) from public.contentflow_tool_execution_queue where project_key=p_project_key and state='pending'),
   'capability_wait',(select count(*) from public.contentflow_tool_execution_queue where project_key=p_project_key and state='blocked' and last_error like 'CAPABILITY_WAIT:%'));
end; $$;
 ...
 with stalled as (
   select r.id,r.backlog_task_id,r.task_key,r.selected_model,r.idempotency_key,r.activity_phase,
          case when r.activity_deadline_at is not null and r.activity_deadline_at<=v_now then 'activity_timeout'
               when r.heartbeat_deadline_at is not null and r.heartbeat_deadline_at<=v_now then 'heartbeat_timeout'
   ...
   where r.project_key=p_project_key and r.status in ('claimed','running') and r.finished_at is null and r.lease_revoked_at is null
     and ((r.activity_deadline_at is not null and r.activity_deadline_at<=v_now) or (r.heartbeat_deadline_at is not null and r.heartbeat_deadline_at<=v_now))
   ...
   update public.contentflow_build_backlog b set status='ready',selected_model=null,next_eligible_at=v_now,updated_at=v_now from revoked s where b.id=s.backlog_task_id and b.status='running' returning s.*
 ) select count(*) into v_count from backlog;
 update public.director_worker_queue q set status='ready',current_task_key=null,last_outcome='durable_activity_recovered',updated_at=v_now where q.status='running' and exists(select 1 from public.contentflow_builder_runs r where r.selected_model=q.model_id and r.finished_at=v_now and (r.error like 'ACTIVITY_TIMEOUT:%' or r.error like 'HEARTBEAT_TIMEOUT:%'));
 get diagnostics v_workers=row_count;
 update public.contentflow_nexo_slots s set released_at=coalesce(s.released_at,v_now),release_reason=coalesce(s.release_reason,'durable_activity_recovered') where s.released_at is null and exists(select 1 from public.contentflow_builder_runs r where r.task_key=s.task_key and r.finished_at=v_now and (r.error like 'ACTIVITY_TIMEOUT:%' or r.error like 'HEARTBEAT_TIMEOUT:%'));
 get diagnostics v_slots=row_count;
 return jsonb_build_object('recovered_runs',v_count,'workers_released',v_workers,'slots_released',v_slots,'ownership_model','durable_execution_v2');
end $$;
...
CREATE FUNCTION public.contentflow_sync_dependency_states(p_project_key text DEFAULT 'contentflow'::text) RETURNS jsonb
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$ declare demoted int:=0; promoted int:=0; begin update public.contentflow_build_backlog b set status='planned',selected_model=null,updated_at=now() where b.project_key=p_project_key and b.status='ready' and exists(select 1 from jsonb_array_elements_text(coalesce(b.depends_on,'[]'::jsonb)) dep(value) where not exists(select 1 from public.contentflow_build_backlog d where d.project_key=b.project_key and d.task_key=dep.value and d.status='completed')); get diagnostics demoted=row_count; update public.contentflow_build_backlog b set status='ready',selected_model=null,updated_at=now() where b.project_key=p_project_key and b.status='planned' and b.task_key not like 'gap_gap_%' and not exists(select 1 from jsonb_array_elements_text(coalesce(b.depends_on,'[]'::jsonb)) dep(value) where not exists(select 1 from public.contentflow_build_backlog d where d.project_key=b.project_key and d.task_key=dep.value and d.status='completed')); get diagnostics promoted=row_count; return jsonb_build_object('demoted_waiting',demoted,'promoted_dispatchable',promoted); end $$;
...
begin
  if new.project_key='contentflow' and new.status='completed' and old.status is distinct from new.status then
    update public.director_help_alerts
     ...
     where project_key='contentflow' and task_key=new.task_key and status='open';

    update public.contentflow_build_backlog p
       set status='ready', updated_at=now(), selected_model=null
     where p.project_key='contentflow'
       ...
       and not exists (
         select 1
           ...
           left join public.contentflow_build_backlog q
             on q.project_key='contentflow' and q.task_key=d.dep
...
begin
  if coalesce(auth.role(),'') <> 'service_role' then raise exception 'service_role_required'; end if;
  if p_project_key <> 'contentflow' then raise exception 'builder_locked_to_contentflow'; end if;
  return query
  with candidate as (
    select b1.id
...
CREATE FUNCTION public.internal_builder_collect(p_request_id bigint) RETURNS text
    ...
    SET search_path TO 'public', 'net'
    AS $$
declare v_d public.contentflow_builder_dispatches%rowtype; v_r net._http_response%rowtype; v_j jsonb; v_run public.contentflow_builder_runs%rowtype; v_ok boolean; v_quality numeric; v_cost numeric; v_model text; v_result text; v_final text;
begin
 select * into v_d from public.contentflow_builder_dispatches where request_id=p_request_id for update; if not found then return 'dispatch_not_found'; end if; if v_d.status<>'pending' then return v_d.status; end if;
 select * into v_r from net._http_response where id=p_request_id; if not found then return 'pending'; end if;
 select * into v_run from public.contentflow_builder_runs where id=v_d.builder_run_id for update; if not found then update public.contentflow_builder_dispatches set status='failed',collected_at=now(),error='run_missing' where request_id=p_request_id; return 'run_missing'; end if;
 if v_run.lease_revoked_at is not null or exists(select 1 from public.contentflow_builder_runs n where n.backlog_task_id=v_run.backlog_task_id and n.id>v_run.id) then
   ...
   insert into public.contentflow_runtime_event_ledger(builder_run_id,task_key,event_type,idempotency_key,actor,payload) values(v_run.id,v_run.task_key,'late_response_fenced',v_run.idempotency_key,'collector',jsonb_build_object('request_id',p_request_id)) on conflict do nothing;
   return 'superseded';
 end if;
 ...
 v_ok:=coalesce((v_j->>'ok')::boolean,false); v_quality:=coalesce((v_j->>'quality_score')::numeric,0); v_cost:=coalesce((v_j->>'cost_usd')::numeric,0); v_model:=nullif(v_j->>'selected_model',''); v_result:=nullif(v_j->>'result','');
 if v_cost<0 or v_cost>0.05 then v_ok:=false; end if; if not v_ok or v_quality<80 then v_final:='failed'; elsif v_run.task_type='code' then v_final:='review_required'; else v_final:='completed'; end if;
...
begin
 ...
 elsif r.action_type='review_gate_reconcile' then detail:=public.contentflow_review_gate_reconcile(i.project_key); ok:=coalesce((detail->>'requeued')::int,0)>0;
 ...
 if ok then
   ...
   update public.director_repair_incidents set status=v_resolved_status,resolved_at=now(),updated_at=now(),executed_action=r.action_type,validation=r.validation_type,outcome='resolved_by_known_recipe',requires_human=false where id=i.id;
...
CREATE FUNCTION public.rara_apply_review_decision_v2(p_builder_run_id bigint, p_claim_token text, p_approve boolean, p_reason text DEFAULT NULL::text) RETURNS jsonb
    ...
    SET search_path TO 'public'
    AS $$
declare q public.contentflow_review_work_queue%rowtype; d jsonb; l jsonb;
begin
 if coalesce(auth.role(),'')<>'service_role' then raise exception 'service_role_required'; end if;
 ...
 d:=public.rara_apply_review_decision(p_builder_run_id,p_approve,p_reason);
 if coalesce((d->>'ok')::boolean,false)=false and p_approve and d->>'reason'='deterministic_evidence_gate_failed' then
   d:=public.rara_apply_review_decision(p_builder_run_id,false,'Deterministic evidence gate failed at commit: '||coalesce(p_reason,''));
 end if;
 ...
 if coalesce(d->>'decision','')='rejected_requeued' then
   l:=public.rara_learn_and_replan_rejection(p_builder_run_id,coalesce(p_reason,''));
   if coalesce(l->>'action','')='owner_required' then
     update public.contentflow_build_backlog b set status='blocked',blocked_reason='OWNER_REQUIRED',selected_model=null,next_eligible_at=null,updated_at=now()
       where b.id=(select backlog_task_id from public.contentflow_builder_runs where id=p_builder_run_id);
     insert into public.director_repair_incidents(project_key,component,error_class,error_fingerprint,symptom,status,risk_level,requires_human,diagnosis,root_cause,proposed_action)
     select project_key,'rara_review','owner_required','rara_owner_required_v1',left(coalesce(p_reason,''),1000),'needs_help','high',true,'RARA rejection requires owner/legal/cost authority','Non-autonomous authority boundary','Request explicit owner/counsel approval'
     from public.contentflow_builder_runs where id=p_builder_run_id
     on conflict do nothing;
   end if;
   d:=d||jsonb_build_object('learning',l);
 end if;
 return d;
end $$;
...
begin
 ...
 set state='pending',claim_token=null,claimed_at=null,available_at=now(),last_error='stale_claim_recovered',updated_at=now()
 where q.state='claimed' and q.claimed_at<now()-interval '5 minutes'
   and exists(select 1 from public.contentflow_builder_runs as r where r.id=q.builder_run_id and r.status='review_required');
 select q.builder_run_id,q.task_key into v_id,v_task
 from public.contentflow_review_work_queue as q
 join public.contentflow_builder_runs as r on r.id=q.builder_run_id
...
CREATE FUNCTION public.rara_classify_rejection(p_reason text) RETURNS text
    ...
    AS $$
 select case
  when lower(coalesce(p_reason,'')) ~ '(legal|counsel|consent|biometric|privacy|purchase|spend|cost authorization|gpu rental|production deploy|irreversible|owner approval)' then 'owner_required'
  when lower(coalesce(p_reason,'')) ~ '(hardcod|run-specific|builder_run_id|evidence id|correlation id)' then 'hardcoded_execution_identity'
  when lower(coalesce(p_reason,'')) ~ '(placeholder|stub|scaffold|empty json|empty object)' then 'placeholder_or_stub'
  when lower(coalesce(p_reason,'')) ~ '(missing evidence|evidence.*insufficient|runtime proof|persisted evidence|acceptance criterion|verification)' then 'acceptance_evidence'
  when lower(coalesce(p_reason,'')) ~ '(contract|interface|integration.*not.*verified|incomplete)' then 'contract_incomplete'
...
begin
  ...
  insert into public.director_repair_incidents(project_key,task_key,component,error_class,error_fingerprint,symptom,evidence,risk_level)
  select p_project_key,b.task_key,'dispatcher','stale_dispatch','stale_dispatch:'||d.request_id::text,'dispatch pending beyond expected window',jsonb_build_object('request_id',d.request_id,'created_at',d.created_at,'builder_run_id',d.builder_run_id,'backlog_task_id',d.backlog_task_id),'low'
  from public.contentflow_builder_dispatches d join public.contentflow_build_backlog b on b.id=d.backlog_task_id
  where d.status='pending' and d.created_at<now()-interval '5 minutes'
...
declare
 r public.contentflow_builder_runs%rowtype;
 b public.contentflow_build_backlog%rowtype;
 cls text; fp text; rule text; corr text; deps_ok boolean:=true; target text; evk text;
begin
 select * into r from public.contentflow_builder_runs where id=p_builder_run_id;
 if not found then return jsonb_build_object('ok',false,'reason','run_not_found'); end if;
 select * into b from public.contentflow_build_backlog where id=r.backlog_task_id for update;
 ...
 fp:='rara_reject_'||cls||'_v1';
 rule:=case cls
  when 'hardcoded_execution_identity' then 'Never emit or hardcode builder_run_id, evidence IDs, correlation IDs, prior-run IDs, UUIDs or execution-specific identifiers inside reusable artifacts. Treat them only as runtime-supplied metadata/parameters.'
  when 'placeholder_or_stub' then 'Do not submit placeholders, empty objects, scaffolds or stubs as completed artifacts. Every required section must contain substantive implementation or explicit bounded contract content.'
  when 'acceptance_evidence' then 'Do not claim execution, deployment or verification without persisted correlated evidence. If evidence is required, produce or route to the approved evidence producer; otherwise keep claims source-only.'
  when 'contract_incomplete' then 'Implement the concrete contract required by acceptance criteria, including deterministic interfaces/tests where required; do not substitute generic abstractions for the requested integration.'
  else 'Address every RARA rejection point explicitly before resubmission and do not repeat previously rejected claims or structures.' end;
 corr:='Auto-replan task using RARA feedback; close retry circuit after contract repair; inject prevention rule into the next execution contract.';
 ...
 values(b.project_key,'rara_review_rejection',fp,'rara_review',left(coalesce(p_reason,''),1000),cls,corr,rule,'builder_run_id='||p_builder_run_id,1,0,0,0.70,'active',now(),now())
 on conflict(project_key,error_fingerprint) do update set last_seen_at=now(),updated_at=now(),occurrences=public.director_error_memory.occurrences+1,symptom=excluded.symptom,root_cause=excluded.root_cause,correction=excluded.correction,prevention_rule=excluded.prevention_rule,evidence=excluded.evidence,status='active',confidence=least(0.99,greatest(public.director_error_memory.confidence,0.70));

 if cls<>'acceptance_evidence' then
   for evk in select evidence_task_key from public.contentflow_evidence_requirements where project_key=b.project_key and backlog_task_id=b.id and source_run_id=p_builder_run_id and status in ('open','task_created') loop
     update public.contentflow_evidence_requirements set status='obsolete',updated_at=now() where project_key=b.project_key and backlog_task_id=b.id and source_run_id=p_builder_run_id and evidence_task_key=evk;
     update public.contentflow_build_backlog set status='deferred',blocked_reason='OBSOLETE_FALSE_EVIDENCE_REQUIREMENT',updated_at=now() where project_key=b.project_key and task_key=evk and status in ('ready','planned','blocked');
     update public.contentflow_build_backlog set depends_on=coalesce(depends_on,'[]'::jsonb)-evk,updated_at=now() where id=b.id;
   end loop;
 end if;

 select not exists(select 1 from jsonb_array_elements_text(coalesce((select depends_on from public.contentflow_build_backlog where id=b.id),'[]'::jsonb)) d(dep)
   where not exists(select 1 from public.contentflow_build_backlog x where x.project_key=b.project_key and x.task_key=d.dep and x.status='completed')) into deps_ok;
 ...
 update public.contentflow_build_backlog set status=target,selected_model=null,quality_score=0,blocked_reason=null,next_eligible_at=now(),description=case when position(rule in coalesce(description,''))>0 then description else coalesce(description,'')||E'\n\n[AUTO-LEARNED RARA PREVENTION RULE] '||rule end,result=coalesce(result,'')||E'\n[AUTO-REPAIR] '||corr,updated_at=now() where id=b.id;
 update public.director_repair_incidents set status='resolved',requires_human=false,resolved_at=now(),updated_at=now(),diagnosis='RARA rejection classified as autonomously correctable: '||cls,proposed_action='Replanned with persistent prevention rule',outcome='auto_replanned_after_rara_rejection' where project_key=b.project_key and status='needs_help' and error_class in ('progress_stall','quality_review','acceptance_evidence');
 insert into public.director_autonomy_events(project_key,event_type,source,assignment_mode,outcome,required_user_intervention,notes,finished_at) values(b.project_key,'rara_rejection_learned','rara_learning','memory_plus_replan','requeued',false,format('run=%s task=%s class=%s fingerprint=%s rule=%s',p_builder_run_id,b.task_key,cls,fp,left(rule,500)),now());
 return jsonb_build_object('ok',true,'action','learned_and_replanned','class',cls,'fingerprint',fp,'task_key',b.task_key,'target_status',target,'prevention_rule',rule);
end $$;
...
begin
  ...
  update public.contentflow_build_backlog b
  set status='ready',selected_model=null,quality_score=0,updated_at=now(),result=coalesce(result,'')||E'\n[RARA] safe requeue after diagnosed failure/timeout'
...
--
-- Name: contentflow_evidence_requirements contentflow_evidence_requirem_project_key_backlog_task_id_r_key; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.contentflow_evidence_requirements
    ADD CONSTRAINT contentflow_evidence_requirem_project_key_backlog_task_id_r_key UNIQUE (project_key, backlog_task_id, requirement_fingerprint);
...
--
-- Name: supabase_realtime contentflow_fresh10_items; Type: PUBLICATION TABLE; Schema: public; Owner: -
--

ALTER PUBLICATION supabase_realtime ADD TABLE ONLY public.contentflow_fresh10_items;


--
-- Name: supabase_realtime contentflow_fresh10_runs; Type: PUBLICATION TABLE; Schema: public; Owner: -
--

ALTER PUBLICATION supabase_realtime ADD TABLE ONLY public.contentflow_fresh10_runs;
...
\unrestrict nizyeI3wwMtX74YBWUwCNk6hkvvAk1s00KbCKIFjuf6JWbJ4vU1cYfk9jmr1RyN
