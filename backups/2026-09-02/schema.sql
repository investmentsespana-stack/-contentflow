...
\restrict rs26Uy70QLtnEweUAQSbArVCbupMmwtvtlm96wcJrg1kp6hu3nMmHx5jv3HWK1b
...
begin
  ...
  for v_action in select trim(value) from jsonb_array_elements_text(coalesce(p_actions,'[]'::jsonb)) loop
    ...
    insert into public.contentflow_build_backlog(project_key,epic,task_key,title,description,task_type,stage,depends_on,team,status,priority,acceptance_criteria,execution_lane,workflow_state)
    values(p_project_key,'external_handoff',v_task_key,left(v_action,180),
      ...
      'general',v_stage,'[]'::jsonb,'social-ops:'||v_source,'ready',least(30,10+v_idx),
      ...
      'llm_artifact','artifact_pending') on conflict(project_key,task_key) do nothing;
...
\unrestrict rs26Uy70QLtnEweUAQSbArVCbupMmwtvtlm96wcJrg1kp6hu3nMmHx5jv3HWK1b
